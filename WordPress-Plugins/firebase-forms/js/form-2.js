		...
		if(imgsnap.val() != null){
			...
			for(var rkey in imgarraydata){
				...
				imghtml += "<div id='"+rkey+"' style='margin-bottom:10px;'>" +
								"<img src='"+record.url+"' id='contestphoto' style='max-width:300px;' />&nbsp;&nbsp;" +											
...
var submitForm2 = function (fl=true){	
	...
	//if imgae uploaded and not going to upload image then dont show error message 
	var isimg = jQuery("#uploadedfile").find("img");
	if(file == undefined && isimg[0] == undefined){
		isimg = false;
	}else{
		isimg = true;
	}
	if(file != undefined){
		...
		function(snapshot) {				
			...
			var imgdesc = document.getElementById("pdesc").value;
			var placesval = jQuery("#places").val();
			if(places == null){
				placesval = "";
			}
			
			var imgdata = {desc:imgdesc, url:downloadURL, createdate: datetime, status: 0, places: placesval};
			var database = firebase.database();			
			firebase.database().ref('wp_fb_images/' +mconid+ '/' + userId).push(imgdata).then(function(imgInfo){				
				stoploading();			
				var imgdata = {desc:imgdesc, url:downloadURL, createdate: datetime, status: 0, places: placesval};
				//imgdata['contestid'] = mconid;
